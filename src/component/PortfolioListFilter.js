...
const PortfolioListFilter = ({collection, toggleFilter, open, selectedTags, filterAction}) => {
  return <div>
            <div className="row center row-nav-animation">
              ...
              <div className="col-sm-12 col-md-10">
                <div className="selectedTags">{selectedTags.map((tag, key)=>
                  <span className="btn btn-mod btn-border btn-circle" key={key} onClick={filterAction.bind(null, 'remove', key)}>{tag.name} <i className="fa fa-close" /></span>
                )}</div>
              </div>
            </div>
